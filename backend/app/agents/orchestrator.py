...
class OrchestratorAdapter(BaseAgentAdapter):
    ...
    async def stream(
        ...
        for chunk in _text_block(next_block_index, _summary_text(tasks, task_states)):
            yield chunk
        next_block_index += 1
        yield StreamChunk(event_type="done", agent_id=self.agent_id, total_blocks=next_block_index)
...
async def _get_sub_adapter(config: Mapping[str, Any], agent_id: str) -> BaseAgentAdapter:
...
def _dependencies_satisfied(
    ...
    return all(task_states.get(task_id) == TaskState.SUCCEEDED for task_id in task.depends_on)
