...
### 3. Seed Built-In Agents When Needed
...
```bash
cd /home/ubuntu/agenthub/backend
ps aux | rg 'uvicorn app.main:app|python -m uvicorn app.main:app' | rg -v rg
...
如果端口已被旧后端占用，只结束确认后的旧 `uvicorn app.main:app --port 8000` 进程。
