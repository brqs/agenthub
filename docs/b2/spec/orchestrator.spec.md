...
| `task_fallback_agent_ids` | `list[str]` | 子任务失败或 artifact 缺失时可尝试的 fallback Agent 列表；默认空。 |
...
7. 子 stream 正常结束后，如果存在明确 artifact path 候选，则只读校验文件是否存在。
8. 子 stream 正常结束且 artifact 存在或无需校验时，任务标记为 `succeeded`。
9. 子 stream 出现 error chunk 或抛异常则任务标记为 `failed`。
10. 子 stream 正常结束但 expected artifact 缺失时，任务标记为 `artifact_missing`。
11. 如果配置开启 per-task fallback，失败或 artifact 缺失任务可进入下一次 attempt。
...
- 默认总预算 `task_result_context_max_chars=4000`。
- 默认单个任务预算 `task_result_item_max_chars=1200`。
...
- 从 `expected_output`、任务 instruction、子 Agent 文本和 tool arguments 中提取明确 path 候选。
- 只检查 workspace root 内的相对路径是否存在。
- 不读取文件内容，不做 mime sniff，不启动 preview。
- 不判断 artifact 是否“正确”或“好看”。
...
- 开启 per-task fallback 后，失败或 `artifact_missing` 可改派 fallback agent。
