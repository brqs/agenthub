...
- Container policy、受控 `ContainerDeployWorker`、queueable dispatcher；当前默认启用 rootless Podman
  容器 worker，runtime 不可用时返回受控 `not_supported`，Docker 仍需显式 trusted override。
...
当前默认使用 rootless Podman 执行受控 build/run，API 初始返回 `queued` 并由 worker 后台推进状态。
runtime 不可用或管理员关闭 worker 时仍返回可解释的 `not_supported`。
...
- 返回后端 `/releases/{release_token}` 代理 URL；容器动态端口只在本机 worker/health check 内使用。
- 支持 stop / cleanup。
- 支持 logs preview。
...
-> 健康检查
-> 生成 release token
-> 回写 deployment(status="published", proxy url, runtime_id, logs)
```
...
DEPLOYMENT_CONTAINER_BUILD_TIMEOUT_SECONDS=180
DEPLOYMENT_CONTAINER_RUN_TIMEOUT_SECONDS=60
...
Docker 仍可作为受信任宿主的显式 override：`DEPLOYMENT_CONTAINER_RUNTIME=docker`
时必须同时设置 `DEPLOYMENT_CONTAINER_TRUSTED_HOST_MODE=true`。
...
5. 如果 container worker 被管理员关闭或 runtime 不可用：返回可解释的 `not_supported`。
6. 默认路径使用 rootless Podman 真实构建运行；Docker 仅在 trusted override 后运行。
...
- `create_deployment(container)` 默认进入 rootless Podman worker；runtime 不可用或被管理员关闭时返回
  `not_supported`；Docker trusted override 后可走 Docker worker。
...
- 2026-06-08 起默认改为 `DEPLOYMENT_CONTAINER_ENABLED=true` + `podman`；新增 runtime preflight，
  runtime 不可用时仍以 `not_supported` 记录环境限制。
...
2026-06-08 后端直连 Deployment API E2E：
