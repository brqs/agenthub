...
`planner_context_max_tokens` defaults to `128000` and may be configured up to
`1000000`. This path is separate from the normal Orchestrator execution context
and is used only when the flow actually enters LLM Planner. The prompt must
preserve the latest user request, available agents with planning profiles,
memory signals, and a recent conversation section with `[Agent: <agent_id>]`
labels for group history.

Explicit `config.tasks` is a deterministic routing contract. When present,
Orchestrator must not detour into previous-output follow-up prompts or generic
clarification/direct-answer branches before executing those tasks. Static E2E
or automation tasks may set `available_agents_authoritative=false`; in that
mode execution/fallback may use `managed_agent_ids` or
`task_fallback_agent_ids`, but the report and task card must still retain
planned/current/final agent evidence.

Fallback is bounded by the current group or explicit E2E escape hatch:

- In normal group conversations, dispatch and fallback must not call agents
  outside `available_agents`.
- In non-authoritative E2E/static automation, fallback may use only the explicit
  managed/fallback whitelist.
- User-visible task cards must render the actual current/final execution agent,
  not only the original planned agent. When `planned_agent_id != final_agent_id`,
  run detail and E2E reports must expose that difference.

The 2026-06-11 robustness E2E matrix validates these rules across parallel
implementation, group attribution, fallback visibility, custom read-only agent
review, context follow-up, browser/deployment repair, and pure dialogue
