...
> 定义 AgentHub 在进入任务规划、单 Agent runtime 和代码产物生成之前，如何判断需求是否足够明确，以及如何向用户逐步追问。
>
> 状态：Implemented MVP
> 最后更新：2026-06-13
...
- 群聊会话包含 `orchestrator` 时，strict 需求对齐由 Orchestrator 统一负责；Orchestrator 在确认边界前不得调度子 Agent。
- 单聊会话开启 strict 时，当前单聊 Agent 负责轻量需求对齐；该对齐通过 ModelGateway/API 生成结构化问题，不启动 Claude/Codex/OpenCode CLI/SDK，不读写 workspace。
- 显式 slash command `/grill-me`、`/grill-with-docs`、`/setup-matt-pocock-skills` 仍然保留，不依赖开关。
- strict 模式下，负责对齐的 Agent 在执行前只问影响执行方向的关键问题；如果需求已经足够明确，则输出假设 brief 并等待用户明确确认。
...
结论：可以应用到 AgentHub 的 Orchestrator 主 Agent，也可以作为单聊 Agent 的 pre-runtime gate；但不应直接照搬为 Claude Code slash command。
...
- 单聊 Agent 已有 direct-chat / runtime prelude，适合在启动 CLI/SDK 前插入轻量 ModelGateway 需求对齐。
...
5. 用户补充后，负责对齐的 Agent 能把新答案与原始请求合并成稳定需求，再进入正常 planner 或单 Agent runtime。
...
- 群聊 / Orchestrator 调度路径不让子 Agent 自行追问用户；用户需求澄清由 Orchestrator 统一处理。单聊 strict 是例外，由当前单聊 Agent 在 runtime 前追问。
...
- 群聊包含 `orchestrator` 且本 turn 为 strict：后端将本轮目标 Agent 收敛为 Orchestrator，由 Orchestrator 输出 `clarification` block。
- 单聊 strict：目标 Agent 自己输出 `clarification` block；如果用户确认继续，才进入该 Agent 的 direct-chat 或 CLI/SDK runtime。
- Orchestrator 已经分发到子 Agent 的 task 必须跳过单聊需求对齐，避免执行阶段二次追问。
...
- Orchestrator runtime context 表明当前消息是子任务执行，而不是用户直接发给单聊 Agent。
...
7. 用户回答“按这个做 / 开始实现 / 按默认开始实现”等明确确认语时，
   负责对齐的 Agent 把原始请求、已确认回答和推荐默认合并成新的 user message。
   群聊 Orchestrator 进入正常 planner；单聊 Agent 才进入 direct-chat 或真实 runtime。
8. 用户提到项目 B 时：
