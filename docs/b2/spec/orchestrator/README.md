...
2. [agent-turn-taking.spec.md](agent-turn-taking.spec.md)
3. [core.spec.md](core.spec.md)
4. [../agent-runtime-adapter.spec.md](../agent-runtime-adapter.spec.md)

修改 Orchestrator 多 Agent 轮流对话 / panel 协作：

1. [agent-turn-taking.spec.md](agent-turn-taking.spec.md)
2. [task-planning.spec.md](task-planning.spec.md)
3. [message-attribution.spec.md](message-attribution.spec.md)
4. [presentation-collapse.spec.md](presentation-collapse.spec.md)
...
- ReAct replanner 已实现为 LLM 控制点；batch-level Re-planner 已可通过
  `orchestrator_batch_replanner_enabled=true` 作为实验开关接入每个 parallel batch
  之后，但默认关闭，当前默认主链仍是静态 DAG 并行 executor。
...
- 自定义 Agent 默认走服务器 Agent 套壳：`create_custom_agent` 可基于 `claude-code`、`codex-helper`、`opencode-helper`
  创建 wrapper，并通过 Skills 资产入口扩展。另有受限 `provider="builtin"` 只读路径，用于用户自建 Review/Reader
  Agent；该路径只允许 `allowed_tools=["read_file"]`，不暴露模型账号、MCP JSON、runtime command、写文件或 bash 权限。
...
- 2026-06-08 pure dialogue group debate repair loop 已通过：`group_dialogue_debate_no_artifacts`
  场景 conversation `a918dc3f-42f0-4e64-8e71-bc6e3f9ed4b4`，`passed=true`；至少两个非 Orchestrator
  child messages 均为 `done`，无 `artifact_missing`，无 `server.js/package.json` 负向约束误判。
