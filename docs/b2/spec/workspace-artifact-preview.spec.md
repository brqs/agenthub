...
| `document` | `README.md`, `report.md`, `brief.pdf`, `brief.docx` | file card / text preview when possible |
| `ppt` | `ppt_outline.json`, `slides.md`, `deck.pptx` | file card / outline text preview |
| `image` | `logo.png`, `diagram.svg` | file card / image preview |
| `archive` | `dist.zip`, `export.tar.gz` | file card / archive metadata |
...
平台内部维护 `.agenthub/artifacts.json` v1，agent runtime 和 workspace file API 都不允许读写
`.agenthub/`。该文件不作为用户可读 workspace 文件暴露，前端只通过受控 API 消费。
...
同一 `path` 去重更新：保留 `created_at`，刷新 `updated_at`。manifest JSON 损坏时平台会恢复为空
v1 manifest；恢复事件只作为平台 warning / memory event，不改变用户消息状态。
...
- `FileBlock.artifact_kind` 使用 `document | ppt | image | archive | code | workflow | other`。
- `FileBlock.path` 保留 workspace 相对路径，`url` 指向 `/api/v1/workspaces/{conversation_id}/files/{path}`。
- `FileBlock` 只承载聊天内即时展示所需字段；`task_id`、`run_id`、`evaluation_status` 和 `evaluation_results` 通过 manifest API 按 `path` 补齐。
- `.md/.txt/.csv/.json/.yaml` 可生成截断 `preview_text`。
- `.pptx` 只做 OpenXML 文本层预览和 slide count，不做版式/动画深度解析。
- 图片只读取安全 metadata 和使用文件 API URL 预览。
- archive 只读取目录 metadata，检查 path traversal，不解压落盘。
...
| 方法 | 路径 | 用途 |
|---|---|---|
| `GET` | `/api/v1/workspaces/{conversation_id}/artifacts` | 只读列出 manifest entries |
...
`GET /api/v1/workspaces/{conversation_id}/files/.agenthub/artifacts.json` 仍然返回
workspace violation，不暴露原始 manifest 文件路径。

2026-06-03 公网 API/SSE E2E 已验证 manifest v1 当前契约：
...
passed: true
covered_kinds: document, ppt, image, archive
checks:
  file_blocks_present: true
  manifest_present: true
  block_manifest_aligned: true
  manifest_has_agent_task_run: true
```
...
- 聊天中 document / PPT / image / archive 产物以 `file` ContentBlock 暴露 `artifact_kind`、`path`、`agent_id` 和 preview metadata。
- 公网 `p1_rich_artifacts` 已验证 persisted message file block 与 manifest entry 的 `path` / `artifact_kind` / `agent_id` 对齐，并且 manifest entry 包含 `task_id` / `run_id`。
- 前端产品化展示、版本历史和局部编辑入口见 [../../frontend/rich-artifact-preview-handoff.md](../../frontend/rich-artifact-preview-handoff.md)，不作为 B2 后端阻塞项。
