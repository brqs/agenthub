...
新增 `docs/b1/spec/stream-error-status.spec.md` 和 B2-06 stream error 协同任务文档，明确 SSE error 状态持久化边界。
...
## 2026-06-02 — Codex 收尾移动端 P0 触屏交互

### 任务
完成移动端 P0 剩余的触屏操作菜单、移动弹窗和窄屏富媒体适配。

### 关键 Prompt
> 开始下一批

### AI 输出摘要
1. ConversationItem 增加手机更多菜单，置顶和归档不再依赖 hover。
2. Agent 消息增加显式触屏 `@Agent` 按钮，Chat Header 增加移动端更多菜单。
3. 底部导航增加账号入口，Settings 使用手机全屏布局，UserMenu 使用底部 sheet。
4. 新建会话弹窗增加手机全屏、滚动内容区和固定安全区底栏。
5. Web Preview 使用手机全屏，DeploymentStatusBlock 操作区允许窄屏换行。

### 人工调整
本次只修改前端与文档，不修改 OpenAPI、后端接口或数据库结构。本地浏览器已完成 `375 × 812` 触屏入口核对；iPhone Safari 和完整真实链路仍需最终验收。

### 经验
响应式布局完成后还要逐项排查 hover 和右键依赖。触屏端必须提供明确可见入口，桌面快捷操作只能作为增强，不能成为唯一路径。
## 2026-06-02 — Codex 实现移动端 P1 PWA 基础能力

### 任务
在移动端 P0 完成后继续开发 P1 PWA，支持安装、静态壳离线浏览、网络状态反馈和版本更新提示。

### 关键 Prompt
> 接下来

### AI 输出摘要
1. 新增 manifest、品牌图标、maskable 图标和 iOS Web App meta，满足移动端安装入口需要。
2. 使用可审计的原生 Service Worker 缓存静态壳、脚本、样式、字体和图标，并显式绕过 `/api` 请求，避免缓存真实 API、SSE、Workspace 和消息数据。
3. 新增 `useNetworkStatus`、全局 `OfflineBanner` 与 PWA 更新提示。
4. 聊天输入框离线时禁用输入和发送，保留当前内存中的已加载消息供查看。
5. 增加离线事件、横幅状态和发送保护测试。

### 人工调整
本次只修改前端与文档，不修改 OpenAPI、后端接口或数据库结构。采用原生 Service Worker，避免新增构建插件和锁文件变化。
...
## 2026-06-03 — Codex 执行 P1 完善项公网 Live E2E

### 任务
对 P1-1 合流消息归属、P1-2 Workflow ContentBlock、P1-3 Agent-to-Agent Review Thread / repair 进行公网后端 API/SSE live E2E 验收，并把证据沉淀到报告和 gap todo。

### 关键 Prompt
> 那开始执行# P1 完善项 Live E2E 执行计划

### AI 输出摘要
1. 扩展 `backend/scripts/orchestrator_live_e2e.py`，新增 `p1_attribution`、`p1_workflow`、`p1_review_thread_repair` 三个场景和独立 report/SSE 输出。
2. 修复 live 中发现的 workflow text fence 落库缺口：普通 text block 中含 workflow fenced YAML 时会保留 text，并追加正式 `workflow` ContentBlock。
3. 修复 review task 被 artifact evaluator 干扰的问题：review 正常完成即解析 `review_outcome`，不会把被审文件当作 review 自己的交付产物评估。
4. 修复 live 脚本临时 patch builtin Orchestrator config 后 restore 的 async engine event-loop 问题，并确认运行后 config 已恢复。
5. 三个公网场景全部通过：
...
### 人工调整
本轮重启本机后端两次加载修复，最终 PID `2872639`，Alembic `5c6d7e8f9012 (head)`，本机和公网 `/health` 均返回 `{"status":"ok"}`。未部署前端；本轮硬验收只覆盖公网后端 API/SSE 和持久化数据。

### 经验
Live E2E 的 prompt 不能用否定形式提旧能力关键词，否则容易触发旧模板路由。Review task 是协作判断，不应复用 implementation artifact evaluator；否则“指出问题”本身会被误判成失败交付。

## 2026-06-03 — Codex 完成 Workflow Runtime / Dry-run MVP

### 任务
实现 P1-B2-01 Workflow runtime / dry-run：在 workflow ContentBlock / preview MVP 基础上补齐本地无副作用 runner、run history / health API，并用公网 API/SSE live E2E 验收。

### 关键 Prompt
> PLEASE IMPLEMENT THIS PLAN: # P1-B2-01 Workflow Runtime / Dry-run 执行计划

### AI 输出摘要
1. 新增 `workspace_workflow_runs` 持久化表、schema、API 和 `WorkspaceWorkflowRuntimeService`。
2. 本地 runner 支持 `trigger`、`task(set_context)`、`assert(equals)`、`end`，执行 DAG topological order，检测 cycle 和 unsupported runtime。
3. Orchestrator 在 `workflow_validation` passed 后自动 dry-run，并将 `workflow dry-run` 结果写入 summary / memory event。
4. SSE 持久化落库前按 workflow `path` 回填 latest run 的 `last_run_id`、`dry_run_status` 和 `health_status`。
5. 扩展 `backend/scripts/orchestrator_live_e2e.py`，新增 `p1_workflow_runtime` 场景；公网 live E2E 已通过：
   - `/tmp/agenthub_p1_workflow_runtime_report.json`
   - `/tmp/agenthub_p1_workflow_runtime_sse.jsonl`

### 人工调整
执行 Alembic `6d7e8f9012ab (head)`，后端从 PID `2872639` 重启到 `2929191`，本机和公网 `/health` 均返回 `{"status":"ok"}`。未部署前端；本轮硬验收为公网后端 API/SSE。

### 经验
Workflow runtime MVP 必须保持无副作用边界。先把可审计的 dry-run、node trace、history 和 health 做稳，比一上来允许 shell/HTTP/platform tool step 更适合作为课程答辩证据。

## 2026-06-03 — Codex 完成 Rich Artifact / Evaluation Repair 公网闭环

### 任务
按 `docs/ai-skills/orchestrator-live-e2e-repair-loop/SKILL.md` 执行 P1-B2-02 / P1-B2-03 公网 API/SSE 验收；失败后继续定位、修复、重启和重跑，直到 rich artifacts 与 evaluation repair 两个场景均通过。

### 关键 Prompt
> PLEASE IMPLEMENT THIS PLAN: # P1-B2-02 / P1-B2-03 Live E2E 修复闭环执行计划

### AI 输出摘要
1. 在 `AGENTS.md` 增加项目级触发规则：涉及 Orchestrator live E2E、B2/P1 公网 API/SSE 验收或“失败继续修继续测直到通过”时，先读并遵循项目内 repair loop skill。
2. 修复 `document_quality` strict evaluator：允许顶层标题由子章节承载内容，拒绝最终文档中的 TODO/placeholder，并跳过 fenced code 中的 placeholder 证据。
3. 调整 `p1_rich_artifacts` live prompt：archive 任务改由 `opencode-helper` 执行 shell tar 生成与 `tar -tf` 验证，避免 `claude-code` Bash approval 卡死。
4. `p1_evaluation_repair` 公网 E2E 通过：`document_quality failed -> reflection -> repair/fallback -> final passed`，manifest 没有 false passed。
5. `p1_rich_artifacts` 公网 E2E 通过：persisted message 和 manifest 覆盖 document / ppt / image / archive，且 `path` / `artifact_kind` / `agent_id` 对齐。

### 证据
- Rich artifacts report：`/tmp/agenthub_p1_rich_artifacts_report.json`
- Rich artifacts SSE：`/tmp/agenthub_p1_rich_artifacts_sse.jsonl`
- Rich artifacts conversation：`c6da3473-b338-4321-ba7d-eb0f877e70ae`
- Evaluation repair report：`/tmp/agenthub_p1_evaluation_repair_report.json`
- Evaluation repair SSE：`/tmp/agenthub_p1_evaluation_repair_sse.jsonl`
- Evaluation repair conversation：`5186e757-6a7c-4d0f-8643-c9b3defbc181`

### 人工调整
本轮只修改后端 evaluator、live E2E 脚本、测试和文档；不改前端 UI、不执行 seed、不部署前端。后端重启后 PID 为 `3192468`，公网 `/health` 返回 `{"status":"ok"}`，本机 health 需使用 `curl --noproxy '*'` 避免代理环境变量影响。
...
## 2026-06-04 — Codex 完成 B2-TODO-05 Deployment / Release 公网 E2E

### 任务
对 Deployment / Release production hardening 做公网后端 direct API E2E 验收，覆盖 production-default
和 trusted Docker demo override 两条路径；远端前端 UI 不作为阻断项。

### 关键 Prompt
> B2-TODO-05 Deployment / Release 公网 E2E 执行计划

### AI 输出摘要
1. 更新 `scripts/deployment_release_api_e2e.py`，适配 container queued-first 状态流，支持
   `AGENTHUB_E2E_EXPECT_CONTAINER_STATUS=not_supported|published|any`，并在报告中记录
   `worker_id`、`attempt_count`、`failure_category`、`last_error_code`、`state_events` 和轮询耗时。
2. 执行本地门禁：deployment/orchestrator 回归 `81 passed`，ruff passed，mypy passed，
   `git diff --check` passed。
3. 执行 Alembic upgrade/current，当前 head 为 `7e8f9012abcd`；重启后端并确认运行实例已加载
   `worker_id/state_events` 新列，PID `3866485 -> 3951595`，本机和公网 health 均正常。
4. Production-default E2E 通过：`/tmp/agenthub_b2_todo_05_prod_default_e2e_report.json`，
   conversation `42b7d9e4-1243-4b4c-9394-1ebb54568ed3`，container 最终 `not_supported`，
   runtime_kind `podman`，preview/static release/source zip/cleanup 均通过。
5. Trusted Docker demo override E2E 通过：`/tmp/agenthub_b2_todo_05_demo_container_e2e_report.json`，
   ...
   healthcheck 和 stop cleanup 均通过。

### 人工调整
Demo override 通过 `systemctl set-environment` 临时注入
`DEPLOYMENT_CONTAINER_ENABLED=true`、`DEPLOYMENT_CONTAINER_RUNTIME=docker`、
`DEPLOYMENT_CONTAINER_TRUSTED_HOST_MODE=true` 等环境变量；验收后已 `unset-environment`
并重启恢复 production-default，最终 PID `3953542`，本机和公网 `/health` 均返回 `{"status":"ok"}`。
本轮未执行 seed，未部署前端，未运行可选 Orchestrator API/SSE 场景。
...
## 2026-06-08 — Codex 实现 Agent 资产表、部署产品化和 Rich Artifact 聚合

### 任务
为四项产品化能力分别补 spec 并实现：自定义 Agent 完整编辑弹窗、knowledge/skill 独立表与版本/使用记录、部署状态时间线/日志/健康/停止重试，以及 Rich Artifact 专用预览与 manifest 聚合视图。

### 关键 Prompt
> 自定义 Agent 完整编辑弹窗，替代 prompt。Agent knowledge/skill 独立表、版本历史、使用记录。部署状态时间线、日志折叠、健康检查、停止/重试更产品化。Rich Artifact 对 document/ppt/image/archive 的专用预览和 manifest 聚合视图。对这四项功能逐个写详细的spec分为前后端部分，然后再根据spec逐个实现。

### AI 输出摘要
新增四份 `docs/spec/*` 规格文档。后端增加 `agent_asset_bindings`、`agent_asset_versions`、`agent_asset_usage_events` 三张表和资产列表/历史/使用记录 API，并保留 `Agent.config` 镜像兼容 B2 runtime。部署服务新增 retry API，前端部署卡片和发布历史补停止 queued、重试、健康检查和真实 state_events 时间线。Workspace 面板新增 Artifact Manifest 聚合视图，Artifact 预览增加 document / ppt / image / archive 摘要。

### 人工调整
没有改变 B2 Adapter 契约；runtime 仍通过统一 context 注入消费用户上传资产。后端数据库集成测试被默认开发库保护拦截，未强制绕过；已完成 py_compile、ruff、前端 tsc 和相关 Vitest。

### 经验
这类跨前后端产品化需求要先把“真相源”说清楚：资产以独立表为准，`Agent.config` 只是兼容镜像；部署 retry 应创建新记录而不是复活旧记录；manifest 不可用时聊天和 workspace 基础预览必须降级可用。
...
## 2026-06-08 — Codex 沉淀前端前景色与对比度验收 Skill

### 任务
仿照 Orchestrator live E2E repair loop，新增一个用于前端前景色、对比度、深浅色主题逐模块逐组件校验的项目 Skill。

### 关键 Prompt
> 仿照 `docs/ai-skills/orchestrator-live-e2e-repair-loop` 制作一个 Skill，用于改前景色和对比度检验，深色模式浅色模式下都需要校验，逐个模块逐个组件校验。

### AI 输出摘要
新增 `docs/ai-skills/foreground-contrast-audit/SKILL.md`，定义触发场景、WCAG 判定标准、模块/组件审查清单、修复策略、contrast 单测要求、浏览器验收矩阵和报告模板；同步更新 `docs/README.md` 和 `docs/frontend/README.md` 暴露入口。

### 人工调整
无。

### 经验
颜色修复不能只看当前截图，应固定成“风险 class 搜索 -> token/组件修复 -> light/dark 浏览器矩阵 -> contrast 单测”的闭环，避免浅色修好后深色回退，或只修一处漏掉同类组件。

## 2026-06-08 — Codex 执行前端前景色与对比度审计

### 任务
按 `docs/ai-skills/foreground-contrast-audit/` 开始校验，覆盖浅色/深色、桌面/移动端和核心页面组件。

### 关键 Prompt
> [foreground-contrast-audit](docs/ai-skills/foreground-contrast-audit/) 开始校验

### AI 输出摘要
创建本地对比度验收会话，覆盖 Text、TaskCard、Process、ToolCall、Code、Diff、DeploymentStatus、File、WebPreview、Attachment 和 workspace 文件树。使用 Playwright 扫描登录、聊天、Agent、归档、Markdown 测试页在 desktop/mobile 与 light/dark 下的真实 computed style；修复浅色 `text-brand-light` 过淡、深色 muted `text-slate-500` 过暗，以及用户紫色气泡被浅色兼容规则误改成深色文字的问题。

### 人工调整
生产 Docker 前端容器已重建；由于 Docker Nginx 当前代理公网后端，本地验收使用 Vite `localhost:5173` 直连本地后端完成，最终报告 `/tmp/agenthub_contrast_audit_report_pass.json`：20 个用例、0 error、0 contrast failure、0 横向溢出。

### 经验
浏览器审计必须确认实际 URL 和采样数量，避免页面被重定向到登录页或 API 被 CORS 阻断后误判通过。全局深浅色兼容规则要保护品牌按钮和用户消息气泡这类反色容器，否则 token 映射会把局部高对比文本改坏。
