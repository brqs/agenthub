...
## 2026-06-08 — Orchestrator Presentation Collapse Markers 启动

### 任务
先补文档契约，再实现前后端标记：前端默认折叠执行过程，只常显成员阶段总结和 Orchestrator
最终回答。

### AI 输出摘要
1. 新增 `docs/b2/spec/orchestrator/presentation-collapse.spec.md`。
2. 约定 ContentBlock 可选 `presentation` metadata，而不是新增 SSE event 或 ContentBlock 类型。
3. `process/tool/intermediate text` 属于可折叠 execution group；`agent_summary` 和
   `final_answer` 常显。
4. 本轮后续会补后端 marker、前端分组折叠、本地测试和公网
   `presentation_collapse_markers_smoke` E2E。

### 完成证据
后端新增 `presentation` marker helper，StreamContentAccumulator 持久化 presentation metadata；
子 Agent text/tool/process/file/deployment 等块标记为 execution group，成员阶段总结标记为
`agent_summary`，Orchestrator 最终回复标记为 `final_answer`。前端 store / stream hook 保留
metadata，ContentRenderer 按 execution group 默认折叠，summary/final answer 常显。
...
backend targeted pytest: 183 passed
backend ruff: passed
backend mypy: passed
