...
| Tool calling / MCP / Memory / Context manager | 仅 [context_compression](../../backend/app/services/context/compression.py) + [conversation_memory](../../backend/app/models/conversation_memory.py) 雏形 | 🟡 部分可复用 |

### 1.4 触发事件

2026-05-26 用户在重新阅读 PDF 后澄清：
- 外部 Agent（Claude Code / Codex / OpenCode）→ 调用已有的 agent runtime SDK 或 CLI runtime
...
│      └── OpenCodeAdapter    → subprocess CLI / JSONL        │
├─────────────────────────────────────────────────────────────┤
...
[backend/app/agents/orchestrator/adapter.py](../../backend/app/agents/orchestrator/adapter.py) 的调度、block_index 重映射、失败降级、fallback adapter **全部不动**。子 Agent 通过新 `BaseAgentAdapter v2` 接口自动升级为真 Agent。Orchestrator 的任务卡片在前端增加"工具调用计数"和"产物文件列表"摘要。

### 2.5 外部 Runtime 接入范围

- **Must**：Claude Code（`claude_agent_sdk`）+ Codex（`openai-agents` Agents SDK）+ OpenCode（subprocess CLI，stdout JSONL 解析）
- **理由**：题目明确要求同时接入 Claude Code / Codex / OpenCode 等主流 Agent 平台；OpenCode 通过 CLI 接入复杂度更高，但仍属于本轮 B2-18 必做范围。
...
| ContextManager | 复用 [context_compression](../../backend/app/services/context/compression.py) | 重新设计 |
...
| Phase 4 | Day 5-6 | CodexAdapter 与 OpenCodeAdapter 接入；Orchestrator 接通真 Agent |
...
| [backend/app/agents/orchestrator/adapter.py](../../backend/app/agents/orchestrator/adapter.py) | `OrchestratorAdapter`（保留公开入口） | 子 Agent 通过 v2 接口自动升级 |
...
| [backend/app/services/context/compression.py](../../backend/app/services/context/compression.py) | `BuiltinAgent.ContextManager` 子组件 | 包装 |
...
| **新建** `backend/app/agents/external/` | ExternalAgentAdapter 实现 | Claude Code / Codex / OpenCode |
...
- 过时历史任务分发记录不再作为当前文档入口；当前事实以 [docs/b2/spec/README.md](../b2/spec/README.md) 为准。
- 不实现一键部署：列为 P2，演示口述
...
- 现有 Orchestrator 规范：[docs/b2/spec/orchestrator/core.spec.md](../b2/spec/orchestrator/core.spec.md)
