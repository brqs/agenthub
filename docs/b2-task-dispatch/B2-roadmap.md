...
| B2-04 | CustomAdapter 委托上游 Provider | P0 | 已拆解，待执行 | `backend/app/agents/adapters/custom.py` | B2-02 / B2-03 |
...
1. 执行 B2-04：实现 CustomAdapter，根据 `config["upstream_provider"]` 委托 Claude/OpenAI。
2. 完成 B2-04 Codex 审阅、Git/PR 和合并。
3. 启动 B2-05 或 B2-06：根据团队协同情况，优先处理 Agent 配置校验或 SSE error 状态持久化。
