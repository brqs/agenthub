...
| B2-05 | Agent 配置校验与内置 Agent 配置对齐 | P0 | 已拆解，待执行 | `backend/app/agents/config_validation.py`, `backend/app/api/v1/agents.py`, `backend/app/schemas/agent.py` | B2-02 / B2-03 / B1 协同 |
...
1. 执行 B2-05：补齐 Agent 配置校验、PATCH config 合并语义和内置 seed 配置对齐。
2. 完成 B2-05 Codex 审阅、Git/PR 和合并。
3. 启动 B2-06：与 B1 协同处理 SSE error 状态持久化，避免 Adapter error 被误标记为 done。
