...
| B2-08 | Orchestrator Spec 与任务拆解 Prompt | P1 | 进行中 | `docs/spec/orchestrator.spec.md`, `docs/b2-task-dispatch/B2-08-orchestrator-spec.md` | B2-04 |
| B2-09 | Orchestrator 子 Agent 顺序调度与 block_index 重映射 | P1 | 已拆解，待 B2-08 Spec 完成后执行 | `backend/app/agents/orchestrator.py`, `backend/tests/test_orchestrator.py` | B2-08 / registry |
...
1. 执行 B2-08：编写 Orchestrator Spec 与任务拆解 Prompt。本任务只做 Spec / Prompt，不实现完整 Orchestrator。
2. 并行准备 B2-09：沉淀 Orchestrator 顺序调度与 `block_index` 重映射任务文档，但实现必须等待 B2-08 Spec 明确。
3. B2-08 Spec 完成后，启动 B2-09：在不改共享契约的前提下实现基础顺序调度和单元测试。
