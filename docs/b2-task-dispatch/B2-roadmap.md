...
| B2-02 | ClaudeAdapter 真实 Anthropic 流式接入 | P0 | 已审阅，待 Git/PR | `backend/app/agents/adapters/claude.py` | B2-01 |
| B2-03 | OpenAIAdapter 真实 OpenAI 流式接入 | P0 | 待拆解 | `backend/app/agents/adapters/openai.py` | B2-01 / B2-02 模式 |
| B2-04 | CustomAdapter 委托上游 Provider | P0 | 待拆解 | `backend/app/agents/adapters/custom.py` | B2-02 / B2-03 |
| B2-05 | Agent 配置校验与内置 Agent 配置对齐 | P0 | 待拆解 | `backend/app/api/v1/agents.py`, `backend/app/seeds/seed_agents.py` | B2-02 / B2-03 / B1 协同 |
| B2-06 | SSE error 状态持久化协同修复 | P0 | 待协同 | `backend/app/api/v1/stream.py` | B2-02，需 B1 协作 |
| B2-07 | ArtifactParser v2：diff/url/web preview 识别 | P1 | 待拆解 | `backend/app/agents/artifact_parser.py` | B2-01，可能涉及 ContentBlock 契约 |
| B2-08 | Orchestrator Spec 与任务拆解 Prompt | P1 | 待拆解 | `docs/spec/orchestrator.spec.md`, `backend/app/agents/orchestrator.py` | B2-04 |
| B2-09 | Orchestrator 子 Agent 顺序调度与 block_index 重映射 | P1 | 待拆解 | `backend/app/agents/orchestrator.py` | B2-08 / registry |
| B2-10 | Orchestrator 失败降级与部分成功输出 | P1 | 待拆解 | `backend/app/agents/orchestrator.py` | B2-09 |
...
1. 完成 B2-02 Git/PR：只提交 ClaudeAdapter、测试、B2-02 文档和协作日志。
2. 启动 B2-03：用 B2-02 的结构实现 OpenAIAdapter，并复用 `StreamingArtifactParser`。
3. 启动 B2-04：实现 CustomAdapter，根据 `config["upstream_provider"]` 委托 Claude/OpenAI。
...
- 当前 B1 SSE 层如果不识别 error chunk，可能仍把 message 状态持久化为 `done`。

该问题需要 B1/B2 协同处理，不应由 B2 在 Provider Adapter PR 中擅自修改 `stream.py`。建议单独开 B2/B1 协作任务：

- 明确 error chunk 出现时如何终止 SSE。
- 明确 message.status 应为 `error`。
- 明确是否持久化已产生的部分 blocks。
...
| PR-B2-05 | `feat/B2-agent-config-validation` | Agent config/model validation，可能协同 B1 |
| PR-B2-06 | `fix/B1-B2-stream-error-status` | SSE error 状态修复，需 B1/B2 协同 |
| PR-B2-08/09 | `feat/B2-orchestrator` | Orchestrator spec + implementation，可视复杂度拆 PR |
