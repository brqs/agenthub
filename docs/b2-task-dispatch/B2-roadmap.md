...
| B2-07 | ArtifactParser v2：diff/url/web preview 识别 | P1 | 已拆解，待执行 | `backend/app/agents/artifact_parser.py`, `backend/app/api/v1/stream.py` | B2-01 / B2-06，复用既有 ContentBlock |
...
1. 执行 B2-07：增强 `StreamingArtifactParser` 对 diff 和独立 URL 的识别，并让 SSE 持久化层保存既有 `diff` / `web_preview` block。
2. 完成 B2-07 Codex 审阅、Git/PR 和合并。
3. 启动 B2-08：编写 Orchestrator Spec 与任务拆解 Prompt。
