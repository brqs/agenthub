...
| B2-11 | Provider retry/timeout/rate-limit 策略 | P1 | 已完成，Codex 审阅通过 | `backend/app/agents/adapters/**`, `backend/tests/test_*adapter.py`, `docs/spec/provider-resilience.spec.md` | B2-02 / B2-03 |
| B2-12 | Adapter E2E smoke tests 与可选真实 API slow tests | P1 | 已完成，Codex 审阅通过 | `backend/tests/**`, `backend/pyproject.toml`, `docs/spec/adapter-smoke-tests.spec.md` | B2-02 / B2-03 / B2-04 / B2-11 |
...
1. 提交 B2-13 文档收尾 PR。
2. 演示前运行 B2-13 中列出的 smoke / ruff / mypy 验证命令。
3. 如需继续增强，优先拆分 Orchestrator 生产 registry 接线协同任务。
...
| PR-B2-11/12 | `feat/B2-provider-resilience` | Provider resilience + adapter smoke tests |
| PR-B2-13 | `docs/B2-final-demo-materials` | B2 演示脚本、答辩材料和架构说明 |
