...
5. **公网只用 HTTPS**：桌面客户端拒绝非回环地址的明文 HTTP。
...
默认保留 `http://localhost:8000` 本地档案。用户可以添加公网 HTTPS 地址、测试连接并切换。远程模式隐藏本地 Docker 控制和“打开本机 Workspace 文件夹”等不成立的操作。
